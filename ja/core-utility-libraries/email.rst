...
``CakeEmail`` はメールを送信するための新しいクラスです。このクラスを使うことで、アプリケーション内の
任意の場所からメールを送信することができます。それに加えて、コントローラから EmailComponent を使うことで、
シェルやモデルからもメールを送信することもできます。

このクラスは :php:class:`EmailComponent` を置き換え、メールの送信をより柔軟に行えるようにします。
たとえば、外部のSMTPサーバやメールサーバの代わりに自身のトランスポーターをメールの送信のために作成できます。
