...
``Email`` はメールを送るための新しいクラスです。このクラスを使うことで、
アプリケーション内の任意の場所からメールを送ることができます。

基本的な使い方
===========

まずはじめに、このクラスをロードしておく必要があります。::

    use Cake\Network\Email\Email;

``Email`` をロードしたあと、以下のようにしてメールを送ることができます。 ::
...
``Email`` のセッターメソッドはクラスのインスタンスを返すので、
メソッドチェーンを使ってプロパティを設定することができます。
...
.. note::
    ...
    deliverability.
configurations separately or load an array of configs.
...
メールのメッセージにファイルを添付することもできます。There are a few
